...
// SearchMemory searches memory entries using FTS5.
func (d *DB) SearchMemory(query string, limit int) ([]Entry, error) {
	safeQuery := sanitizeFTSQuery(query)
	rows, err := d.sql.Query(`
		SELECT m.id, m.text, m.tags, m.source, m.created_at, m.access_count
		FROM memory m
	...
	for rows.Next() {
		var e Entry
		if err := rows.Scan(&e.ID, &e.Text, &e.Tags, &e.Source, &e.CreatedAt, &e.AccessCount); err != nil {
			return nil, err
		}
...
func (d *DB) ListMemory(limit int) ([]Entry, error) {
	rows, err := d.sql.Query(`
		SELECT id, text, tags, source, created_at, access_count
		FROM memory
...
func (d *DB) ListSessions(limit int) ([]Session, error) {
	rows, err := d.sql.Query(`
		SELECT id, started_at, ended_at, cwd, model, tokens_in, tokens_out
		FROM sessions
