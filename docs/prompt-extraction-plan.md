...
- **~150 lines of Go prompt text deleted** across 6 source files
- **~7 new .md files** (~95 lines of markdown)
- **~7 new accessor functions** in `prompts/prompts.go` (~50 lines of Go)
- **Net:** roughly neutral on line count, significantly better separation of concerns

### 7. Tool descriptions and schemas (~estimated 400 lines)

**Source:** `internal/tools/*.go` — every tool's `Description()` string and `Schema()` JSON raw message.

**Create:** `internal/prompts/tools/` directory with one `.md` file per tool (e.g., `read.md`, `write.md`, `edit.md`, `grep.md`, `patch.md`, `sed.md`, `go_refactor.md`, etc.).

Each file contains the tool's human-readable description and parameter documentation in markdown. The JSON schema remains generated by the tool's `Schema()` method (the schema is machine-facing via the MCP protocol), but the parameter descriptions embedded in the schema are LLM-facing and benefit from externalization.

**Affected files:**
- 20+ tool source files: replace inline `Description()` return with `return prompts.ToolDescription("read")`
- `internal/prompts/prompts.go` — add `tools/*.md` to embed, expose `ToolDescription(name string) string`
- Tool schema JSON: extract parameter `description` fields into the markdown files for LLM readability

**Effort:** Medium. Repetitive but mechanical — each tool follows the same pattern. Can be done incrementally tool by tool.
