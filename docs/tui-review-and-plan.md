...
- [ ] **P5.3 — Extensible commands.** Allow MCP tools to register commands (e.g., `:memory search ...`, `:skill list`). Add a `RegisterCommand(name, desc string)` method to the Model.
...
|-----|--------|
| `Ctrl+C` | Quit |
| `Esc` | Back (dismiss command mode / model mode / question / search / help; never quits) |
| `?` | Help screen |
| `?` | Help screen |
| `:` | Command mode (:help, :model, :compact, etc.) |
| `/` | Search in chat |
...
## 9. Implementation Record (July 2025)

All five phases were implemented on branch `tui-improvements` (7 commits).

### Changes from original plan

| Planned | Implemented | Notes |
|---------|-------------|-------|
| Footer hint bar with `/help` prefix | `? help · / search · ctrl+y copy · ctrl+t reasoning · ctrl+c quit` | Footer uses `key.Binding` + `help.Model` |
| `?` help screen | Rounded-border overlay with grouped keybindings (Navigation, Actions, Input, Commands, System) | Dismissed by any key |
| `hjkl`, `gg`, `G` for viewport | `j`/`k` scroll, `gg`/`G` jump, `ctrl+u`/`ctrl+d` page | Also: `↑`/`↓`/`PgUp`/`PgDn`/`Home`/`End` |
| `/` for search | In-viewport search: type to filter, `n`/`N` next/prev match, `Esc` dismiss | Match count shown in indicator line |
| `Ctrl+R` → `Ctrl+T` reasoning | Done | |
| Command prefix `:` | Switched from `/` to `:` (colon) | `/` conflicted with search; `:` follows k9s/vim convention |
| `Esc` = cancel/back | Cancels command mode, model mode, question modal, search, help; never quits | `Ctrl+C` is the only quit key |
| Semantic theme tokens | `Theme` struct → `ApplyTheme()`; `NO_COLOR` support; light/dark auto-detect | Named presets in `namedThemes` map |
| Banner toggle | `/banner` → `:banner` command | Compact header when hidden |
| Ephemeral status | `SetEphemeral()` with ~3s auto-clear via spinner tick | Green status line for feedback |
| Minimum size check | Terminal < 60×20 shows red error message | |
| Token-rate throttling | `needsRefresh` flag flushed on spinner tick (~15 Hz) | From 80+ rebuilds/sec |
| Extensible commands | `RegisterCommand(name, desc)` method | |
| Panic recovery + SIGTSTP | `defer recover()` + signal handlers in `runTUI()` | |
| Golden-file tests | Skipped | teatest setup deferred |

### New files

| File | Purpose |
|------|---------|
| `internal/tui/theme.go` | Theme struct, presets, `ApplyTheme()`, `DetectTheme()`, `NO_COLOR` |
| `internal/tui/keymap.go` | Declarative `key.Binding` keymap, `footerKeyMap` for `help.Model` |
