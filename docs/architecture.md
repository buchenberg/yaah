...
**Data flow:** `SubAgentStartEvent` and `SubAgentEndEvent` are published to the broker and delivered to `Model.HandleEvent`. The start event appends one `Message` with role `"subagent"` (`SubRole` = role key, `Content` = task prompt, `SubRunning` = true). The end event finds the most recent running message for that role and transitions it in place (`SubRunning` = false, duration, error) — no separate end message is appended. The `spawn_subagent` `ToolEndEvent` is not rendered as a tool message either: its result is attached to the matching sub-agent message (`SubResult`), shown via the line's expand toggle (see [TUI component system](./tui-components.md)).
